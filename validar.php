...
class Consultas
{
			...
			if (empty($val2)) {
				...
				if ($tam_rut< 7) {
					...
					if($dvr == strtoupper($dv)){
						...
						$dbh = Database::getInstance();
						$consulta1 = $dbh -> prepare("SELECT autoexcluidos_scj.ae_nombre as nombre, autoexcluidos_scj.ae_email as email, autoexcluidos_scj.ae_tel as tel, autoexcluidos_scj.ae_tele_movil as telmo
						...
						if($resultado){
							...
							if ($resultado) {
								...
								$consulta4 = $dbh -> prepare("UPDATE log SET autoexc = :v_2, cli_pep = :v_3, estado_ticket = :v_4 WHERE rut_ingre = :v_n");
	...
	try {
		$cbd = Database::getInstance();
		$ingre_pasp = $cbd -> prepare("INSERT INTO log(rut_ingre,hora,fecha,autoexc,cli_pep,n_entrada,estado_ticket)VALUES(:v_r,:v_h,:v_f,:v_ae,:v_pp,:v_t,:v_et)");
	...
	public function contarEntradas(){
		...
		$cuenta_personas->bindValue(':v_h',$this->hoy);
